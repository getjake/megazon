...
handler.get(async (req, res) => {
  ...
  db.disconnect();
  if (product) {
    res.send(product.reviews); // reviews -> mongoose.schema
  } else {
...
handler.use(isAuth).post(async (req, res) => {
  ...
  if (product) {
    ...
    if (existReview) {
      // already a review -> update the single review text and rating
      await Product.updateOne(
        { _id: req.query._id, "reviews._id": existReview._id },
        {
          $set: {
      ...
      // update the reviews array and the overall rating
      ...
      await product.save();
      await db.disconnect();
      res.status(201).sned({ message: "Review submitted" });
    }
  } else {
    await db.disconnect();
    res.status(404).send({ message: "Product Not Found" });
